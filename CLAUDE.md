...
- **Version comparison:** `VersionDiff` renders **Monaco's DiffEditor** (the VS Code diff, themed Monokai) — `@monaco-editor/react` + `monaco-editor` bundled **locally** (not the default CDN — same-origin/offline, ADR-023/024) and **lazy-loaded** (`MonacoVersionDiff` is its own chunk) so only the version-history screen pays Monaco's weight.
- **Tests:** co-located `*.test.tsx` (vitest + Testing Library); avoid asserting deep Radix Select/Dialog interactions in jsdom — prefer rendered text/roles. **Monaco can't run in jsdom**: mock `@monaco-editor/react` (stub `DiffEditor`), and `monaco-editor` + its `?worker` are aliased to `src/test/monaco` stubs in `vite.config.ts` (`test.alias`); the real editor is verified via the build + a browser check.
