...
/**
 * Minimal in-memory fake of the slice of EntityManager the Knowledge repositories use
 * (upsert / persist / findOne / find / flush) plus a where-matcher that understands the few
 * operators these repos issue ($ne, $in, $like). `persist` stages without committing, mirroring
 * the version store's append-inside-the-item-transaction behaviour; `flush` makes staged
 * records visible. The real MikroORM behaviour (and tenant filtering) is covered by the
 * Postgres integration/migration check.
 */
class FakeEntityManager {
  ...
  private static matches(entity: EntityRecord, where: Record<string, unknown>): boolean {
    for (const key of Object.keys(where)) {
      if (!FakeEntityManager.matchesCondition(entity[key], where[key])) {
        return false;
      }
    }
    return true;
  ...
  private static matchesCondition(actual: unknown, expected: unknown): boolean {
    if (FakeEntityManager.isOperatorObject(expected)) {
      return FakeEntityManager.matchesOperator(actual, expected);
    }
    return actual === expected;
  }

  private static matchesOperator(actual: unknown, expected: Record<string, unknown>): boolean {
    if ("$ne" in expected) {
      return actual !== expected["$ne"];
    }
    if ("$in" in expected) {
      const candidates = expected["$in"] as ReadonlyArray<unknown>;
      return candidates.includes(actual);
    }
    if ("$like" in expected) {
      const pattern = String(expected["$like"]).replaceAll("%", "");
      return String(actual).includes(pattern);
    }
    return false;
  }

  private static isOperatorObject(value: unknown): value is Record<string, unknown> {
