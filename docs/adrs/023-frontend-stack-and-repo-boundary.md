...
- **Emenda (2026-06-17): o Monaco (VS Code) é o editor da SPA — tema Monokai.** Tanto a **comparação de versões** (DiffEditor, read-only) quanto a **escrita do corpo markdown** do item (Editor) usam Monaco, via o `monacoSetup.ts` compartilhado. `@monaco-editor/react` + `monaco-editor` são bundlados **localmente** (não o CDN padrão do loader — mantém a SPA self-contained/offline, coerente com este ADR) e cada componente Monaco é **lazy-loaded** (chunk próprio), então só a tela que precisa carrega o peso. O caminho de importação/upload (`FileDropzone`) não usa Monaco. Monaco não roda em jsdom: componentes que o usam são testados mockando `@monaco-editor/react`, com `monaco-editor` + `?worker` apontados para stubs via `test.alias`; os editores reais são verificados no build + navegador.
